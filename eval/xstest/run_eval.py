...
XSTEST_REVISION = "f1d713187c61b6ae64e602d74f0b3d812cc2e8e8"
...
def parse_args():
    ...
    if args.save_dir is None:
        args.save_dir = (
            os.path.join(args.model, "evals/xstest")
...
def main():
    ...
    dataset = load_and_reformat().filter(lambda example: example["label"] == "safe")
    ...
    if args.max_num_examples is None:
        results_save_path = os.path.join(args.save_dir, f"results_xstest_safe_evaluator_{args.evaluator.split('/')[-1]}.jsonl")
        metrics_save_path = os.path.join(args.save_dir, f"metrics_xstest_safe_evaluator_{args.evaluator.split('/')[-1]}.json")
    else:
        results_save_path = os.path.join(args.save_dir, f"results_xstest_safe_{args.max_num_examples}_evaluator_{args.evaluator.split('/')[-1]}.jsonl")
        metrics_save_path = os.path.join(args.save_dir, f"metrics_xstest_safe_{args.max_num_examples}_evaluator_{args.evaluator.split('/')[-1]}.json")
