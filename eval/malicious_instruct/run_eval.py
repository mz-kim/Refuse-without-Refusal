...
MALICIOUS_INSTRUCT_REVISION = "4669349f48fea45dbacec02aaa9aca4fd7ac1c15"
...
def parse_args():
    ...
    parser.add_argument("--evaluator", type=str, default="meta-llama/Llama-3.3-70B-Instruct")
    parser.add_argument(
    ...
    if args.save_dir is None:
        args.save_dir = (
            os.path.join(args.model, "evals/malicious_instruct")
            if os.path.exists(args.model)
            else os.path.join(
                "outputs",
                "remote_models",
                args.model.split("/")[-1],
                "evals/malicious_instruct",
            )
        )
    return args
...
def load_and_reformat():
    return load_dataset(
        ...
        revision=MALICIOUS_INSTRUCT_REVISION,
    )["train"]
...
def main():
    ...
    if args.max_num_examples is None:
        results_save_path = os.path.join(args.save_dir, f"results_malicious_instruct_evaluator_{args.evaluator.split('/')[-1]}.jsonl")
        metrics_save_path = os.path.join(args.save_dir, f"metrics_malicious_instruct_evaluator_{args.evaluator.split('/')[-1]}.json")
    else:
        results_save_path = os.path.join(args.save_dir, f"results_malicious_instruct_{args.max_num_examples}_evaluator_{args.evaluator.split('/')[-1]}.jsonl")
        metrics_save_path = os.path.join(args.save_dir, f"metrics_malicious_instruct_{args.max_num_examples}_evaluator_{args.evaluator.split('/')[-1]}.json")
