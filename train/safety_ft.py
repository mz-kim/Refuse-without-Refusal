...
def parse_args():
    ...
    parser.add_argument("--config_dir", default="training_configs")
    parser.add_argument("--continue_training", action="store_true")
    parser.add_argument("--attn_implementation", default="flash_attention_2")
    args = parser.parse_args()

    if "gemma-2" in args.model.lower():
...
def process_training_data(dataset, tokenizer):
    def construct_chat(example, tokenizer=None, user_role='user', assistant_role='assistant'):
        messages = [
            {"role": user_role, "content": example["instruction"]},
            {"role": assistant_role, "content": example["response"]},
        ]
    ...
    dataset = dataset.map(construct_chat, fn_kwargs={"tokenizer": tokenizer}).shuffle(seed=0)
    return dataset

def get_output_path(model_name, dataset_path):
    model_name = f"{model_name.split('/')[-1]}_qlora"
...
def prepare_training(
    ...
    training_args = yaml_loader(os.path.join(config_dir, "qlora.yaml"))
    training_args["output_dir"] = output_path
    training_args = TrainingArguments(**training_args)
    peft_config = LoraConfig(
        **yaml_loader(os.path.join(config_dir, "peft/qlora.yaml"))
    )
    bnb_config = BitsAndBytesConfig(
        **yaml_loader(os.path.join(config_dir, "peft/qlora_bnb.yaml"))
    )
...
def main():
    ...
    output_path = get_output_path(args.model, args.dataset_path)
    print(f"Saving model to {output_path}")
    model, tokenizer, training_args, peft_config = prepare_training(
        args.model,
        args.config_dir,
        output_path,
    ...
    train_dataset = process_training_data(train_dataset, tokenizer)
